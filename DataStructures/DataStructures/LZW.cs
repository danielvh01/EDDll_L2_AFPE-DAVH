    ...
    public class LZW : ILZWCompressor
    {
        ...
        public byte[] Compress(byte[] text)
        {
            ...
            if (text != null || text.Length != 0)
            {
                ...
                {
                    result[i + 2 + dictionaryCharCant] = (byte)Convert.ToInt32(binaryCode.Substring(i * 8, 8), 2);
                }
                return result;
        ...
        public byte[] Decompression(byte[] compressedText)
        {
            ...
            previous = Convert.ToInt32(binaryText.Substring(0, bytesPerCharacter), 2);
            character = dictionary[previous];
            result += character;
            for (int i = 1; i < binaryText.Length / bytesPerCharacter; i++)
            {
                current = Convert.ToInt32(binaryText.Substring(i * bytesPerCharacter, bytesPerCharacter), 2);
                if (current != 0)
                {
